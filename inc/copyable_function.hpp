...
template <typename T>
inline constexpr bool sbo{sizeof(T) <= sizeof(storage_t::sbo) && std::is_nothrow_move_constructible_v<T>};
...
inline auto nop_manage(storage_t*, storage_t*, mode) { return true; }
...
struct vtable final {
    ...
    static void move_ctor(const vtable*& lhs_vptr,
    ...
    static void move_assign(const vtable*& lhs_vptr,
                            storage_t&     lhs_storage,
                            const vtable*& rhs_vptr,
                            storage_t&     rhs_storage) noexcept {
    ...
    static void
    swap(const vtable*& lhs_vptr, storage_t& lhs_storage, const vtable*& rhs_vptr, storage_t& rhs_storage) noexcept {
        if (&lhs_storage == &rhs_storage)
            return;
    ...
    template <bool Copyable, typename T, typename... A>
    static auto init_functor(storage_t& storage, A&&... args) -> const vtable* {
    ...
    static auto init_empty() noexcept -> const vtable* {
...
class invoker {
    ...
    template <typename T>
    static auto move(T&& t) noexcept -> decltype(auto) {
        if constexpr (Move)
            return std::move(t);
        else
            return (t);
    ...
    static auto get(const_<storage_t>* ctx) noexcept -> move_<const_<T>> {
        return move(*reinterpret_cast<const_<T>*>(SBO ? ctx->sbo : ctx->ptr));
    }

  public:
    using dispatch_type = std::conditional_t<Noexcept,
                                             Result (*)(const_<storage_t>*, Args...) noexcept,
                                             Result (*)(const_<storage_t>*, Args...)>;

    template <typename T, bool SBO>
    static auto functor(const_<storage_t>* ctx, Args... args) noexcept(Noexcept) -> Result {
...
struct invocable_using<Result(Args...)> {
    template <typename... T>
    static constexpr bool is_invocable_using{std::is_invocable_r_v<Result, T..., Args...>};
};

template <typename Result, typename... Args>
struct invocable_using<Result(Args...) noexcept> {
    template <typename... T>
    static constexpr bool is_invocable_using{std::is_nothrow_invocable_r_v<Result, T..., Args...>};
};
...
template <typename Result, typename... Args>
struct traits<Result(Args...)> final : invocable_using<Result(Args...)>,
...
template <typename Result, typename... Args>
struct traits<Result(Args...) const> final : invocable_using<Result(Args...)>,
...
template <typename Result, typename... Args>
struct traits<Result(Args...) noexcept> final : invocable_using<Result(Args...) noexcept>,
...
template <typename Result, typename... Args>
struct traits<Result(Args...) const noexcept> final : invocable_using<Result(Args...) noexcept>,
...
template <typename... Ts>
inline constexpr bool is_move_only_function_specialization_v{is_move_only_function_specialization<Ts...>::value};
...
template <typename... Ts>
inline constexpr bool is_copyable_function_specialization_v{is_copyable_function_specialization<Ts...>::value};
...
template <typename T>
inline constexpr bool is_in_place_type_t_specialization_v{is_in_place_type_t_specialization<T>::value};
} // namespace internal_function

template <typename Signature>
class move_only_function<Signature> final
    : internal_function::function_call<move_only_function<Signature>, Signature> {
    ...
    template <typename... T>
    static constexpr bool is_invocable_using{traits::template is_invocable_using<T...>};

    template <typename VT>
    static constexpr bool is_callable_from{is_invocable_using<typename traits::template quals<VT>> &&
  ...
  public:
    ...
    template <
        typename F,
        typename = std::enable_if_t<(!std::is_same_v<move_only_function, std::remove_cvref_t<F>> &&
                                     !internal_function::is_in_place_type_t_specialization_v<std::remove_cvref_t<F>> &&
                                     is_callable_from<std::decay_t<F>>)>> // TODO: [C++20] replace with
                                                                          // concepts/requires-clause
    move_only_function(F&& func) {
        using VT = std::decay_t<F>;
        static_assert(std::is_constructible_v<VT, F>);
        if constexpr (std::is_function_v<std::remove_pointer_t<F>> || std::is_member_pointer_v<F> ||
                      internal_function::is_move_only_function_specialization_v<std::remove_cvref_t<F>>) {
            if (!func)
                ...
                vptr = vtable::template init_functor<false, VT>(storage, std::forward<F>(func));
                             ...
                             /*TODO: [C++23] use using instead of typedef here...*/ std::is_same_v<
                                 std::remove_cvref_t<F>,
                                 copyable>) { // QoI: prevent double-wrapping
    ...
    explicit move_only_function(std::in_place_type_t<T>, A&&... args) {
        static_assert(std::is_same_v<T, std::decay_t<T>>);
        vptr = vtable::template init_functor<false, T>(storage, std::forward<A>(args)...);
    }

    template <typename T, typename U, typename... A>
        requires(std::is_constructible_v<std::decay_t<T>, std::initializer_list<U>&, A && ...> &&
                 is_callable_from<std::decay_t<T>>)
    explicit move_only_function(std::in_place_type_t<T>, std::initializer_list<U> ilist, A&&... args) {
        static_assert(std::is_same_v<T, std::decay_t<T>>);
    ...
    friend auto operator==(const move_only_function& self, std::nullptr_t) noexcept -> bool { return !self; }
};
...
class copyable_function<Signature> final : internal_function::function_call<copyable_function<Signature>, Signature> {
  ...
  public:
    ...
    copyable_function(F&& func) {
        ...
        static_assert(std::is_constructible_v<VT, F>);
        if constexpr (std::is_function_v<std::remove_pointer_t<F>> || std::is_member_pointer_v<F> ||
                      internal_function::is_copyable_function_specialization_v<std::remove_cvref_t<F>>)
            if (!func) {
                vptr = vtable::init_empty();
                return;
            }
    ...
    template <typename T, typename... A>
        requires(std::is_constructible_v<std::decay_t<T>, A && ...> && is_callable_from<std::decay_t<T>>)
    explicit copyable_function(std::in_place_type_t<T>, A&&... args) {
        static_assert(std::is_same_v<T, std::decay_t<T>>);
    ...
    template <typename T, typename U, typename... A>
        requires(std::is_constructible_v<std::decay_t<T>, std::initializer_list<U>&, A && ...> &&
                 is_callable_from<std::decay_t<T>>)
    explicit copyable_function(std::in_place_type_t<T>, std::initializer_list<U> ilist, A&&... args) {
        static_assert(std::is_same_v<T, std::decay_t<T>>);
        vptr = vtable::template init_functor<true, T>(storage, ilist, std::forward<A>(args)...);
    }

    copyable_function(const copyable_function& other) : vptr{other.vptr} {
    ...
    friend void swap(copyable_function& lhs, copyable_function& rhs) noexcept { lhs.swap(rhs); }

    friend auto operator==(const copyable_function& self, std::nullptr_t) noexcept -> bool { return !self; }
};
} // namespace p2548
